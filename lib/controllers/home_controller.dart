...
class HomeController extends GetxController {
  ...
  void _loadUsers() {
    _users.bindStream(
      _firestoreService.getAllUsersStream().map((users) {
  ...
  String formatLastMessageDate(DateTime? time) {
    if (time == null) {
      return '';
    }
  ...
  List<ChatModel> _applyUnreadFilter(List<ChatModel> chats) {
    final currentUser = _authController.user?.uid;

    if (currentUser == null) {
      return [];
    }
    return chats.where((chat) => chat.getUnreadCount(currentUser) > 0).toList();
  }
  ...
  List<ChatModel> getRecentChats({int limit = 10}) {
    final recentChats = _applyRecentFilter(chats);
    ...
    try {
      ...
      final result = await Get.dialog<bool>(
        AlertDialog(
          ...
          content: Text(
            'Are you sure you want to delete ${otherUser?.displayName ?? 'this chat'} chat',
          ),
          actions: [
            TextButton(
